 ...
 */
  ...
  {
	IO.put("USAGE: java acdc.ACDC  <.ta or .rsf input file> <.rsf output file> [-/+patterns] [options]",0);
 ...
 */
  private static boolean matches(String s1, String s2)
 {
  ...
  public static void main (String [] args)
  {
    ...
    final int MAX_ARGS = 7; //max number of arguments
	...
	for (int i= 2; i<args.length; i++)
	{
		...
		{
			inducer = new FullOutput(root, null);
		}
...
//    Pattern r = new ReverseOrphanAdoption(root);
    ...
    while (iv.hasNext())
    {
		...
		p.execute();
		if (gui) {
			displayTree(root, "After " + p.getName());
    }	}

	// Take care of any objects that were not clustered

	Pattern c = new ClusterLast(root);
	IO.put("Executing " + c.getName() + " pattern...",1);
	c.execute();
	if (gui) {
		displayTree(root, "After " + c.getName());
	}
	
    // Create output file
	IO.put("Executing " + inducer.getName() + " inducer...",1);
	inducer.execute();
	IO.put("Creating output...",1);
	output.writeOutput(outputName, root);

	if (gui) {
		displayTree(root, "After " + inducer.getName());
	}

	IO.put("Finished!",1);
  }

	private static void displayTree(DefaultMutableTreeNode root, String title) 
	{
